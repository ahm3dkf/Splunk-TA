...
Splunk.Module.UnixBaseAbstractFactoryFactory = $.klass(Splunk.Module, {
    ...
    */
    ...
    RemoteStorage: function(name, moduleId, allowMultiple, moduleType){
        ...
        function getUrl() {
            return Splunk.util.make_url(['splunkd', '__raw', 'servicesNS', encodeURIComponent(Splunk.util.getConfigValue('USERNAME')), app, 'configs', 'conf-'+conf_name, Splunk.util.getCurrentView()+'?output_mode=json'].join('/'));
        }

        function getSetupUrl() {
            return Splunk.util.make_url(['splunkd', '__raw', 'servicesNS', encodeURIComponent(Splunk.util.getConfigValue('USERNAME')), app, 'configs', 'conf-'+conf_name].join('/'));
        }
